...
API_URL = "http://localhost:50001/scan/"  # Адрес сервера
...
# Функция распознавания QR-кода с изображения
def decode_qr_code(image_bytes):
    ...
    if decoded_objects:
        return decoded_objects[0].data.decode("utf-8")  # Берём первый код
    return None
...
# Хэндлер обработки ссылки с кодом
...
async def process_qr_text(message: Message):
    ...
    qr_code = message.text.split("=")[-1]  # Извлекаем код из ссылки
